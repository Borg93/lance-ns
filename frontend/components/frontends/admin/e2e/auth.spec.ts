...
test('signed-out: a page navigation redirects to /auth/login?redirect=<original path>', async ({
	...
	const res = await page.request.get('/admin/audit', {
		maxRedirects: 0,
		headers: { accept: 'text/html' },
	});
	expect(res.status()).toBe(302);
	expect(res.headers()['location']).toBe('/auth/login?redirect=%2Fadmin%2Faudit');
});

test('signed-out: API routes are EXEMPT — fetch clients get their status/JSON, never a redirect', async ({
	page,
}) => {
	const res = await page.request.get('/admin/api/projects', { maxRedirects: 0 });
