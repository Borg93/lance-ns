...
export function enter(opts: { y?: number; delay?: number; duration?: number } = {}): Attachment<HTMLElement> {
	...
	return (node) => {
		if (reduced()) return;
		const tw = gsap.from(node, { y, autoAlpha: 0, duration, delay, clearProps: 'transform,opacity,visibility' });
		return () => tw.kill();
	};
}

/** Stagger the element's direct children in on mount (for groups that mount together). */
export function stagger(opts: { y?: number; each?: number; delay?: number } = {}): Attachment<HTMLElement> {
...
export function bar(pct: number): Attachment<HTMLElement> {
	return (node) => {
		...
		const tw = gsap.to(node, { width: `${pct}%`, duration: 0.55, ease: 'power2.out', overwrite: 'auto' });
		return () => tw.kill();
	};
}
...
export function breathe(active: boolean): Attachment<HTMLElement> {
	return (node) => {
		...
		const tw = gsap.to(node, { opacity: 0.45, duration: 0.75, repeat: -1, yoyo: true, ease: 'sine.inOut' });
		return () => {
			tw.kill();
			gsap.set(node, { opacity: 1 });
		};
...
export function pop(key: unknown): Attachment<HTMLElement> {
	...
	return (node) => {
		if (reduced()) return;
		const tw = gsap.fromTo(node, { scale: 0.94 }, { scale: 1, duration: 0.4, ease: 'back.out(1.8)', clearProps: 'scale' });
		return () => tw.kill();
	};
}
