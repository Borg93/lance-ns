...
`;
  ...
  async function initGpu(canvas: HTMLCanvasElement): Promise<GpuHandles | null> {
    ...
    const nodePl = dev.createRenderPipeline({
      ...
      vertex: {
        ...
        entryPoint: 'vsNode',
        buffers: [
          { arrayStride: 8, stepMode: 'instance', attributes: [{ shaderLocation: 0, offset: 0, format: 'float32x2' }] },
          { arrayStride: 4, stepMode: 'instance', attributes: [{ shaderLocation: 1, offset: 0, format: 'float32' }] },
          { arrayStride: 4, stepMode: 'instance', attributes: [{ shaderLocation: 2, offset: 0, format: 'unorm8x4' }] },
          { arrayStride: 4, stepMode: 'instance', attributes: [{ shaderLocation: 3, offset: 0, format: 'uint8x4' }] },
        ],
      },
    ...
    const ubo = dev.createBuffer({ size: 48, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
    const mk = (pl: GPURenderPipeline) =>
      dev.createBindGroup({ layout: pl.getBindGroupLayout(0), entries: [{ binding: 0, resource: { buffer: ubo } }] });
  ...
  function ensure(buf: GPUBuffer | null, byteLength: number, capacity: number): [GPUBuffer, number] {
    const dev = device!;
    const size = Math.max(byteLength, 4); // GPU buffers must be non-empty
    if (buf && size <= capacity) return [buf, capacity];
    if (buf) buf.destroy();
    return [dev.createBuffer({ size, usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST }), size];
  }
  ...
  function screenToData(cssX: number, cssY: number): [number, number] {
    ...
    return [(cssX * dpr - resW * 0.5) / scale + center[0], (cssY * dpr - resH * 0.5) / scale + center[1]];
  }
  ...
  $effect(() => {
    ...
    return () => {
      ...
      for (const b of [posBuffer, sizeBuffer, colorBuffer, stateBuffer, edgeBuffer, uniformBuffer]) b?.destroy();
  ...
  function onPointerMove(e: PointerEvent): void {
    ...
    if (drag) {
      ...
      drag.lastX = cx;
      drag.lastY = cy;
      if (Math.abs(cx - drag.startX) > CLICK_SLOP || Math.abs(cy - drag.startY) > CLICK_SLOP) drag.moved = true;
