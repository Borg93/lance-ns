...
| 9 | **Versioned authz-model migration** (`ACTIVE_MODEL_VERSION` + idempotent `migrate()`) | AUTHZ / GOVERNANCE_P1 | `model.json` loaded with no versioning, no migration hooks (services/catalog/auth/). | Add `ACTIVE_MODEL_VERSION` + versioned schema files + idempotent `migrate()` recording applied version (Lakekeeper migration.rs:11-18,142-163). Mandatory **before** the 3-axis (teams×projects×layers) model introduces new types. | **P1** | M |
...
| 18 | **`fga model test` in CI / pre-commit** | AUTHZ | model.fga.yaml has comprehensive check/list_objects tests but no automated runner. | Add a CI step running `fga model test services/catalog/auth/model.fga.yaml`; keep model.fga / model.fga.yaml / model.json in sync. Status: GOOD, just gate it. | P2 | S |
