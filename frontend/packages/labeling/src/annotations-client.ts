...
export interface SavePayload {
  ...
  inserts: InsertRow[];
  geometry: Array<{ id: string; x: number; y: number; width: number; height: number; polygon: number[] }>;
...
export function buildSavePayload(args: {
  ...
  geoEdits: ReadonlyMap<number, { x: number; y: number; w: number; h: number; polygon?: number[] | null }>;
