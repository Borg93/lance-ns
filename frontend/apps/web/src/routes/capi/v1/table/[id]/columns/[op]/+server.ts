...
// an explicit allowlist (add|alter|drop → the catalog's {op}_columns; field-meta|table-meta → the per-field
// and table-level metadata routes for the #74 tail). Forwards ONLY the signed-in user's bearer, so the
