<script lang="ts">
	...
	import { fetchTables, fetchWarehouse, type Warehouse } from '$lib/catalog';
	import { namespaceOfTable, stageOf } from '$lib/stage';
...
<div class="page">
	<header>
		<a class="back" href={`${base}/warehouses`}>Warehouses</a>
		<span class="sep">/</span>
	...
	{:else}
		<section>
			...
			<div class="facts mono">
				<span
					>project <a class="plink" href={`${base}/projects/${encodeURIComponent(wh.project)}`}
						>{wh.project}</a
					></span
				>
			...
			{:else}
				<ul class="list">
					{#each namespaces as { ns, count, info } (ns)}
						<li>
							<a class="row" href={`${base}/namespaces/${encodeURIComponent(ns)}`}>
