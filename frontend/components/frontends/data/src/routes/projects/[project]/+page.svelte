<script lang="ts">
	...
	import { fetchProject, type ProjectSummary } from '$lib/catalog';
...
<div class="page">
	<header>
		<a class="back" href={`${base}/projects`}>Projects</a>
		<span class="sep">/</span>
				...
				<table>
					...
					<tbody>
						{#each detail.warehouses as w (w.id)}
							<tr>
								<td>
									<a class="mono whlink" href={`${base}/warehouses/${encodeURIComponent(w.id)}`}
										>{w.id}</a
									>
								</td>
