...
describe('JWT + session', () => {
	...
	test('sessionFromTokens falls back through preferred_username → email → sub for the display name', () => {
		expect(sessionFromTokens(jwtWith({ sub: 'u2', preferred_username: 'pu' }), 'AT').name).toBe('pu');
	...
	test('isExpired honors exp; exp=0 (no claim) is treated as live', () => {
		expect(isExpired({ sub: 'u', name: 'n', email: null, accessToken: 'a', expiresAt: 100 }, 200)).toBe(true);
		expect(isExpired({ sub: 'u', name: 'n', email: null, accessToken: 'a', expiresAt: 300 }, 200)).toBe(false);
		expect(isExpired({ sub: 'u', name: 'n', email: null, accessToken: 'a', expiresAt: 0 }, 200)).toBe(false);
	});
});
