...
To fix a failure: ``cd frontend && bun run gen:openapi && bun run gen:types`` and commit the result.
"""
...
def test_committed_openapi_contract_covers_the_live_app() -> None:
    ...
    assert not missing_schemas, (
        f"frontend/openapi.json is stale — missing schemas {sorted(missing_schemas)}; regenerate."
    )
