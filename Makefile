...
frontend: ## Run the frontend CI gate (svelte-check + tsgo + bun unit tests + oxlint + rsvelte-fmt) hermetically via Dagger
...
deadcode: ## Dead-code sweep (vulture). Decorator-invoked symbols are IGNORED and reviewed knowns are whitelisted, so output ~0 == a REAL dead symbol surfaces instead of drowning in noise.
	...
	@cd frontend && bunx knip --no-exit-code 2>/dev/null || echo "  (frontend: oxlint no-unused-vars gates unused imports/vars)"
