...
| 7 | **OpenTelemetry Collector** (OTel operator) | the redundant vmagent (removed) + eventually **Vector** | The estate has three collection paths — apps OTLP-direct, **Vector** for no-SDK infra-pod logs, and (briefly) a **vmagent** for Dapr `:9090` metrics. One Collector subsumes all three: `otlp` receiver ← apps, `filelog` ← infra logs (retires Vector), `prometheus` receiver + k8s SD ← Dapr/DCGM metrics (retired vmagent). vmagent was **removed** as a premature third collector; the DaprConsumerWedge rule is defined + promtool-proven and evaluates the moment the Collector feeds `dapr_*` into GreptimeDB. Converge on ONE collector at operator-adoption; Vector stays until then (it's the only infra-log path today). |
