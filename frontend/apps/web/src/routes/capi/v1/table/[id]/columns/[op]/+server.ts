...
// an explicit allowlist (add|alter|drop → the catalog's {op}_columns). Forwards ONLY the signed-in user's
// bearer, so the catalog's writer-tier gate (can_write_data) is enforced against a real user; an anonymous
// visitor on an OIDC tier is refused here.
