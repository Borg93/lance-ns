...
<div class="flex min-h-0 flex-1 flex-col overflow-hidden text-xs" data-testid="annotation-table">
	<div class="overflow-auto">
		<table class="w-full border-collapse">
			<thead
				class="bg-muted/70 text-muted-foreground sticky top-0 z-10 text-left text-[10px] uppercase backdrop-blur"
			>
				<tr>
					{#each cols as c (c.key)}
						<th class={cn('px-2 py-1.5 font-medium', c.num && 'text-right')}>
							...
							>
								{c.label}
								{#if sortKey === c.key}
									{#if sortDir === 1}<ChevronUp class="size-3" />{:else}<ChevronDown
											class="size-3"
										/>{/if}
								{/if}
							</button>
			...
			<tbody>
				{#each sorted as r (r.index)}
					<tr
						class={cn(
							'border-border/50 hover:bg-muted/50 cursor-pointer border-b',
					...
					>
						...
						<td class="text-muted-foreground px-2 py-1">{r.shape || '—'}</td>
						<td class="px-2 py-1">
						...
						>
						<td class="text-muted-foreground px-2 py-1 text-right tabular-nums"
							>{fmt(r.uncertainty)}</td
						>
					</tr>
				{:else}
					<tr
						><td colspan={cols.length} class="text-muted-foreground px-3 py-6 text-center"
							>No annotations</td
						></tr
					>
				{/each}
