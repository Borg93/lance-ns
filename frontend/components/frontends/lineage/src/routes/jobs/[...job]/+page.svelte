...
<div class="page">
	<a class="back" href="{base}/jobs"><ArrowLeft size={12} /> jobs</a>
	<header>
		...
		<section {@attach enter({ y: 6 })}>
			...
			<div class="rel-group">
				...
				{#each upstream as u (u)}
					<a class="rel-chip mono" href={`${base}/datasets/${encodeURIComponent(u)}`}>{u}</a>
				{:else}
			...
			<div class="rel-group">
				...
				{#each downstream as d (d)}
					<a class="rel-chip mono" href={`${base}/datasets/${encodeURIComponent(d)}`}>{d}</a>
				{:else}
					...
					{#if (r.outputs ?? []).length}
						<div class="outs">
							{#each r.outputs ?? [] as out (out)}
								<a class="rel-chip mono" href={`${base}/datasets/${encodeURIComponent(out)}`}
									>{out}</a
								>
							{/each}
						</div>
